...
JNIEXPORT jbyteArray JNICALL Java_ntrusign_Ntrusign_InitialKey(JNIEnv *env,
        jobject obj,
        jstring jstr) {
   ...
   jcharArray kuri = (*env)->NewByteArray(env, CRYPTO_PUBLICKEYBYTES+CRYPTO_SECRETKEYBYTES);
   (*env)->SetByteArrayRegion(env, kuri, 0, CRYPTO_PUBLICKEYBYTES, pk);
   (*env)->SetByteArrayRegion(env, kuri, CRYPTO_PUBLICKEYBYTES, CRYPTO_SECRETKEYBYTES, sk);
